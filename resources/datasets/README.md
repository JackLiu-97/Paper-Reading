...
| ICLR 2024 | ICLR 评审分析；评分、置信度和决策趋势分析 | 自整理；来源为 SEA_data | 待核查；按 OpenReview 与原始公开数据约束使用 | ICLR 204 评审数据；包含评审相关数据，pdf，Nougat对论文解析mmd | [百度网盘 ICLR 2024](https://pan.baidu.com/s/1LN0VOs7AHrUrpLhkyGSdfg)，提取码：`8994` | 课题组自整理 | 包含三类形态：原始评审数据、统一结构化后的历年评审数据、LLM 抽取的优点/缺点/重要性/问题-作者回复 rebuttal 对及满意度。 |
